...
After lint and risk pass, call `apply_rule` before finalizing a candidate. A failed candidate validation, or terminal evidence that is incomplete, is report-only: keep the result partial/browser-unverified and never relabel it as a reasoning fallback.
