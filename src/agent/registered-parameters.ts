 ...
 *
 * `lookup_rule_guidance`, `select_environment` and `launch_browser` have a
 * {@link TOOL_PARAMETER_SCHEMAS} entry, but it is the WIDENED stub shape for the sessions that
 * never register the tool — every property optional, so a call reaches the gate refusal instead of
 * bouncing at validation. These registrations are the real tools and require what the runtime
 * requires.
 *
...
> = {
    ...
    [ToolName.LaunchBrowser]: FIX_TOOL_PARAMETER_SCHEMAS[ToolName.LaunchBrowser],
